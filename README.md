...
Диктовка с глобальным хоткеем: запись с микрофона → STT (локальный **faster-whisper/Vosk** или облачный **OpenAI/Groq/Deepgram/Google STT/Yandex SpeechKit**) → опциональная постобработка через LLM → вставка текста в активное приложение.
...
**Документация:** этот `README.md` — основная точка входа (обзор, запуск, сборка, troubleshooting). Технические детали: [docs/README.md](docs/README.md) · [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md) · [docs/CONFIGURATION.md](docs/CONFIGURATION.md) · [docs/BUILD_CROSS_PLATFORM.md](docs/BUILD_CROSS_PLATFORM.md) · [assets/models/README.md](assets/models/README.md)
...
| Пакет | Роль |
|--------|------|
| `faster-whisper`, `vosk`, `numpy`, `scipy` | Локальный STT |
...
| `app/providers/` | STT и LLM: `faster_whisper_provider`, `vosk_transcription_provider`, `openai_whisper_provider`, `groq_whisper_provider`, `deepgram_transcription_provider`, `google_speech_transcription_provider`, `yandex_speech_transcription_provider`, `openai_llm_provider`, `litellm_llm_provider`. |
...
`Info.plist`: **LSUIElement**, описания для микрофона, Apple Events и универсального доступа.

Иконки сборки:
- исходник бренда: `assets/icons/app_icon.png` (рекомендуется 1024x1024);
- `build.py` на macOS автоматически генерирует `assets/icons/app_icon.icns`;
- `build.py` на Windows автоматически генерирует `assets/icons/app_icon.ico`;
- каталог `assets/icons/` включается в `dist`, чтобы Linux-launcher (`.desktop`) мог использовать ту же иконку.
