...
### Управление секретами в Dashboard

Вкладка Settings позволяет:

- сохранять ключ в `.env.secrets` (каталог поддержки приложения);
- удалять ключ только из `.env.secrets` (и текущей сессии процесса);
- удалять ключ «везде» (`.env.secrets`, локальные `.env`, `os.environ` в текущем процессе);
- видеть источник значения (`.env.secrets`, локальный `.env`, `environment`).

Логика реализована в `ConfigManager` (`set_secret`, `delete_secret`, `secret_source`).
